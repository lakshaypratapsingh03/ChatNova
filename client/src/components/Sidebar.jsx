...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const menuItem = "flex items-center gap-3 p-3 mt-1 rounded-lg cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/30 hover:border-purple-500 hover:scale-[1.02]";


  return (
    <div className={`flex flex-col h-screen min-w-72 w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-10 ${isMenuOpen ? 'max-md:translate-x-0' : 'max-md:-translate-x-full'}`}
    >
      {/*logo*/}
      <img src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark} alt="" className='w-full max-w-48' />

      {/*New Chat Button*/}
      <button onClick={createNewChat} className={`flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer ${menuItem}`}>
        <span className='mr-2 text-xl'>+</span>New Chat
      </button>

      {/*Search Chats*/}
      <div className={`flex items-center gap-3 p-3 mt-4 border border-gray-700 md:border-gray-300 dark:border-white/20 rounded-md ${menuItem}`}>
        <img src={assets.search_icon} alt="" className='w-4 not-dark:invert' />
        <input id="sidebar-search" name="sidebar-search" onChange={(e) => setSearch(e.target.value)} value={search} type="text" placeholder='Search Chats' className='text-xs placeholder:text-gray-500 md:text-gray-900 outline-none' />
      </div>
      ...
      {/* Recent Chats */}
      {chats.length > 0 && <p className='mt-4 mb-2 text-sm cursor-pointer'>Recent Chats</p>}
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
            <div onClick={() => { navigate('/'); setSelectedChat(chat); setIsMenuOpen(false) }}
              key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-700 md:border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
              <div>
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className={`flex items-center gap-3 p-3 mt-4 border border-gray-700 md:border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all${menuItem}`}>
        <img src={assets.diamond_icon} className='w-4.5 dark:invert pb-2' alt="" />
        <div className='flex flex-col text-sm'>
          <p>Credits : {user?.credits}</p>
          <p className='text-xs text-gray-00 md:text-gray-400'>Purchase credits to use ChatNova</p>
        </div>
      </div>


      {/* Dark Mode Toggle  */}
      <div className='flex items-center justify-between gap-3 p-3 mt-4 border border-gray-700 md:border-gray-300 dark:border-white/15 rounded-md '>
